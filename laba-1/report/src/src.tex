...
используемое исключительно для сортировки. Константы \texttt{CHAR\_FOR\_DAYS}, \texttt{CHAR\_FOR\_MONTH},
\texttt{CHAR\_FOR\_YEARS} и \texttt{DIGITS\_NUM} задают параметры формата даты (длину дня, месяца, года)
и основание системы счисления (десятичные цифры). Вся совокупность записей хранится в векторе пар
\texttt{std::pair<Date, std::string>}, где второй элемент --- строка значения.
...
года, используя форматную строку. Такой подход позволяет корректно выделять части даты
...
функция \texttt{radix\_sort}, которой передаётся ссылка на вектор данных. Отсортированная последовательность выводится с
